...
#@export var change_layer : Array[bool]:
	#set(v):
		#update_layer_switches()
		#change_layer = v
		#
#@export var switch_layers : Array[Layer2D] = [] :
	#set(v):
		#switch_layers = v
		#update_layer_switches()
		
#func update_layer_switches() -> void:
	#for i in switch_layers:
		##if i is Layer2D:
		#change_layer.append(false)

@export_category("NE DEBUG")
...
var last_x_vel_y : float = 0
var last_floor_angle : float = 0
var last_real_floor_angle : float = 0
var last_wall_angle : float = 0
var last_real_wall_angle : float = 0
var rad_entering_angle : float = 0
var last_floor_normal : Vector2 = Vector2.ZERO
var last_wall_normal : Vector2 = Vector2.ZERO
var x_vel : Vector2 = Vector2.ZERO
var y_vel : Vector2 = Vector2.ZERO
...
var is_releasing_vertically : bool = false
	...
	if !is_movement_blocked:
		match state:
			sm.GROUND:
				...
				if (!$Rotatable/Casts/UpCast0.is_colliding() and !$Rotatable/Casts/UpCast1.is_colliding() and !is_sliding ) and !(!$Timers/TurningTimer.is_stopped() and is_running):
					jump_start(delta)
				floor_detaching()
				if !is_on_wall():
					sprite_leveling()
			...
			sm.AIR:
				...
				if $Timers/FirstJumpStateTimer.is_stopped() and $Timers/SecondJumpStateTimer.is_stopped():
					if !is_running:
						...
						if is_releasing:
							if Vector2.UP.dot(velocity) > 0.5:
								movement_in_air_state_when_running()
							#if velocity.x < (speed - 150) and velocity.x > -(speed + 150):
								#movement_in_air_state_when_running()
					is_jumping = false
					if is_releasing and $Rotatable.rotation < deg_to_rad(135):
						jump_start(delta)
					falling(delta)
				else:
					movement(delta, 0.5, 0.5)
...
func state_machine(delta : float):
		...
		match state:
			...
			sm.AIR:
				dust_paticle_emitters_stop_emitting()
				if !is_running and !is_releasing and $Timers/FloorStickBlockingTimer.is_stopped():# or is_releasing:
					if $Timers/FirstJumpStateTimer.is_stopped() and $Timers/SecondJumpStateTimer.is_stopped():
	...
	if ((Input.get_axis("LEFT" + get_player_index(), "RIGHT" + get_player_index()) != 0) or (is_running) or (is_sliding)) and $Timers/SlideTimer.is_stopped() and $Timers/TurningTimer.is_stopped():
		if state != sm.AIR:
			if is_running:
				...
				x_vel = lerp(x_vel, last_floor_normal.rotated(PI/2 * last_true_axis) * speed, 0.3)
			else:
				#$Anim.play("move")
		...
		if state != sm.AIR:
			if is_running:
				...
				#if get_floor_angle() > deg_to_rad(40) or is_on_wall():
					#y_vel.x = 10 * get_real_floor_angle() / get_floor_angle()
				#else:
...
func movement_in_air_state_when_running() -> void:
	...
	x_vel.x += last_true_axis * MAX_SPEED/24
	#last_true_axis_changing()
	#x_vel.x = Input.get_axis("LEFT" + get_player_index(), "RIGHT" + get_player_index()) * MAX_SPEED
	...
	if Input.is_action_pressed("RUN" + get_player_index()) and ( (!($Rotatable/Casts/LeftCast.is_colliding() and last_true_axis == -1 and speed <= 700) and !($Rotatable/Casts/RightCast.is_colliding() and last_true_axis == 1 and speed <= 700)  ) or state == sm.WALL_SLIDING ):
		if is_on_floor() or is_on_wall():
			...
			if $Timers/TurningTimer.is_stopped():
				if !is_sliding:
					speed = move_toward(speed, MAX_SPEED, 750 * get_physics_process_delta_time())
...
func floor_detaching() -> void:
	if not(is_on_floor()) and !( $Rotatable/Casts/DownCast0.is_colliding() and $Rotatable/Casts/DownCast1.is_colliding() ):
...
func jump_start(delta : float) -> void:
	if ( Input.is_action_just_pressed("JUMP" + get_player_index())) or (!$Timers/JumpInputBuffer.is_stopped() ) and is_sliding == false:# and get_floor_angle() > PI/2):
		...
		if ( last_floor_angle > deg_to_rad(45) ) and is_sliding == false:
			...
			if get_floor_angle() > PI/4 and state == sm.GROUND:
				$Anim.play("walljump")
			...
			#if abs($Rotatable.rotation) < deg_to_rad(135) and abs($Rotatable.rotation) > deg_to_rad(45):
...
func slide_dash_start() -> void:
		...
		#is_sliding = true
	if (Input.is_action_pressed("3ACTION" + get_player_index()) and is_running and speed > MAX_SPEED - 10) or is_in_slide_area == true:
		if $Timers/SlideDashTimer.is_stopped():
			$Anim.play("slide_start")
		#elif $Anim.:
			#$Anim.play("slide")
		$Timers/SlideDashTimer.start()
...
func attack() -> void:
	if !is_grinding and $Anim.current_animation != "grind":
		if Input.is_action_just_pressed("1ACTION" + get_player_index()) and $Timers/AttackTimers/AttackTimer.time_left < $Timers/AttackTimers/AttackTimer.wait_time/100 and state != sm.GHOST and $Timers/TurningTimer.is_stopped():
			#$Rotatable/AttackAreas/AttackArea/Shape.disabled = false
			var random_attack_sprite_rotation_offset : float = RandomNumberGenerator.new().randf_range(-1, 1)
			$Rotatable/AttackSprite.rotation += PI/100 * random_attack_sprite_rotation_offset
			...
			y_vel.y -= 2 * g.GRAVITY
			if !$Timers/FirstJumpStateTimer.is_stopped():
				await $Timers/FirstJumpStateTimer.timeout
				stop_jump_timers()
			if !$Timers/SecondJumpStateTimer.is_stopped():
				await $Timers/SecondJumpStateTimer.timeout
				stop_jump_timers()
			await $Timers/AttackTimers/AttackTimer.timeout
			$Rotatable/AttackSprite.rotation = 0
...
func sprite_rotation_reset_immidiate() -> void:
	$Sprite.rotation = 0
...
func _remote_ch_screen_shake(force : float = 0.4, time : float = 0.2) -> void:
	ch.start_screen_shake(force, time)
